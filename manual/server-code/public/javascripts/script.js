...
var settingPath = 'http://' + siteUrl + '/data/setting.json';
var lastChangePath = 'http://' + siteUrl + '/data/ledStatus.json';
	...
	apiCall(ledStatePath).then(function(res) {
		...
		if ( data.ledState == "blink" ) {
			...
			apiCall(settingPath).then(function(res) {
				...
				setTimeout(function() {
					if ( checker = true ) {
						postCall('http://' + siteUrl + '/stateOne');
					}
				}, stateOne);

				setTimeout(function() {
					if ( checker = true ) {
						postCall('http://' + siteUrl + '/stateTwo');
					}
				}, stateTwo);

				setTimeout(function() {
					if ( checker = true ) {
						postCall('http://' + siteUrl + '/stateThree');
					}
				}, stateThree);
